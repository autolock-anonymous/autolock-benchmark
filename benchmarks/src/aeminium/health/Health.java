...
public class Health {
  ...
  @Perm(requires="none(sim_population_ratio) * unique(current) * immutable(id) * immutable(level) * unique(seed) * none(IQ) * none(sim_seed) * immutable(root) * full(id) * unique(sim_id) * immutable(seed) * immutable(home_village) * share(population) * immutable(population) * pure(hosp) * none(personnel) * pure(free_personnel) * none(sim_cities) * share(children) * immutable(children) in alive",ensures="none(sim_population_ratio) * unique(current) * immutable(id) * immutable(level) * unique(seed) * none(IQ) * none(sim_seed) * immutable(root) * full(id) * unique(sim_id) * immutable(seed) * immutable(home_village) * share(population) * immutable(population) * pure(hosp) * none(personnel) * pure(free_personnel) * none(sim_cities) * share(children) * immutable(children) in alive") public static Village allocateVillage(  int level,  int vid,  Village back,  boolean isAe,  int threshold){
    ...
    try {
      ...
      children_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock.writeLock().unlock();
    }
  }
  public static ReentrantReadWriteLock getSim_seedLock(){
    Cloner cloner=new Cloner();
    sim_seedLock=cloner.deepClone(sim_seedLock);
    Cloner cloner=new Cloner();
    sim_seedLock=cloner.deepClone(sim_seedLock);
    return sim_seedLock;
  }
  public ReentrantReadWriteLock getChildren_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock(){
    Cloner cloner=new Cloner();
    children_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock=cloner.deepClone(children_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock);
    Cloner cloner=new Cloner();
    children_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock=cloner.deepClone(children_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock);
    return children_current_free_personnel_home_village_hosp_id_level_population_root_seed_sim_idLock;
  }
  public static ReentrantReadWriteLock getSim_population_ratioLock(){
    Cloner cloner=new Cloner();
    sim_population_ratioLock=cloner.deepClone(sim_population_ratioLock);
    Cloner cloner=new Cloner();
    sim_population_ratioLock=cloner.deepClone(sim_population_ratioLock);
    return sim_population_ratioLock;
  }
  public static ReentrantReadWriteLock getIQLock(){
    Cloner cloner=new Cloner();
    IQLock=cloner.deepClone(IQLock);
    Cloner cloner=new Cloner();
    IQLock=cloner.deepClone(IQLock);
    return IQLock;
  }
  public static ReentrantReadWriteLock getSim_citiesLock(){
    Cloner cloner=new Cloner();
    sim_citiesLock=cloner.deepClone(sim_citiesLock);
    Cloner cloner=new Cloner();
    sim_citiesLock=cloner.deepClone(sim_citiesLock);
    return sim_citiesLock;
  }
  public static Village getCurrent(){
    Cloner cloner=new Cloner();
    current=cloner.deepClone(current);
    Cloner cloner=new Cloner();
    current=cloner.deepClone(current);
    return current;
  }
  public ReentrantReadWriteLock getPersonnelLock(){
    Cloner cloner=new Cloner();
    personnelLock=cloner.deepClone(personnelLock);
    Cloner cloner=new Cloner();
    personnelLock=cloner.deepClone(personnelLock);
    return personnelLock;
  }
}
