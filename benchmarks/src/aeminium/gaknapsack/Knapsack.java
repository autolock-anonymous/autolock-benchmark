...
public class Knapsack {
  ...
  @Perm(requires="unique(items) in alive",ensures="unique(items) in alive") Knapsack(){
  ...
  @Perm(requires="unique(rand) in alive",ensures="unique(rand) in alive") public static void resetSeed(){
    has_randLock.writeLock().lock();
    rand=null;
    has_randLock.writeLock().unlock();
  }
  @Perm(requires="pure(rand) in alive",ensures="pure(rand) in alive") public static Indiv createRandomIndiv(  Indiv ind){
  ...
  @Perm(requires="pure(rand) * share(has) in alive",ensures="pure(rand) * share(has) in alive") public static Indiv recombine(  Indiv ind,  Indiv p1,  Indiv p2){
    try {
      has_randLock.writeLock().lock();
  ...
  @Perm(requires="none(SIZE_LIMIT) * full(fitness) in alive",ensures="none(SIZE_LIMIT) * full(fitness) in alive") public static void evaluate(  Indiv indiv){
  ...
  @Perm(requires="none(size) * pure(has) * none(value) * none(items) * none(weight) in alive",ensures="none(size) * pure(has) * none(value) * none(items) * none(weight) in alive") public static int[] phenotype(  Indiv indiv){
    int value=0;
    int weight=0;
  ...
  @Perm(requires="pure(rand) * share(has) in alive",ensures="pure(rand) * share(has) in alive") public static void mutate(  Indiv indiv){
    has_randLock.readLock().lock();
    has_randLock.writeLock().lock();
  ...
  public static MersenneTwisterFast getRand(){
    Cloner cloner=new Cloner();
    rand=cloner.deepClone(rand);
    Cloner cloner=new Cloner();
    rand=cloner.deepClone(rand);
    return rand;
  }
  public static ReentrantReadWriteLock getHas_randLock(){
    Cloner cloner=new Cloner();
    has_randLock=cloner.deepClone(has_randLock);
    Cloner cloner=new Cloner();
    has_randLock=cloner.deepClone(has_randLock);
    return has_randLock;
  }
  public static ReentrantReadWriteLock getItemsLock(){
    Cloner cloner=new Cloner();
    itemsLock=cloner.deepClone(itemsLock);
    Cloner cloner=new Cloner();
    itemsLock=cloner.deepClone(itemsLock);
    return itemsLock;
  }
  public ReentrantReadWriteLock getFitnessLock(){
    Cloner cloner=new Cloner();
    fitnessLock=cloner.deepClone(fitnessLock);
    Cloner cloner=new Cloner();
    fitnessLock=cloner.deepClone(fitnessLock);
    return fitnessLock;
  }
  public ReentrantReadWriteLock getValueLock(){
    Cloner cloner=new Cloner();
    valueLock=cloner.deepClone(valueLock);
    Cloner cloner=new Cloner();
    valueLock=cloner.deepClone(valueLock);
    return valueLock;
  }
  public static ReentrantReadWriteLock getSIZE_LIMITLock(){
    Cloner cloner=new Cloner();
    SIZE_LIMITLock=cloner.deepClone(SIZE_LIMITLock);
    Cloner cloner=new Cloner();
    SIZE_LIMITLock=cloner.deepClone(SIZE_LIMITLock);
    return SIZE_LIMITLock;
  }
  public ReentrantReadWriteLock getWeightLock(){
    Cloner cloner=new Cloner();
    weightLock=cloner.deepClone(weightLock);
    Cloner cloner=new Cloner();
    weightLock=cloner.deepClone(weightLock);
    return weightLock;
  }
