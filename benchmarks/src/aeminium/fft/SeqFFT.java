...
public class SeqFFT {
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static Complex[] sequentialFFT(  Complex[] x){
    int N=x.length;
...
class Client {
  ...
  @Perm(requires="pure(x) * immutable(DEFAULT_SIZE) in alive",ensures="pure(x) * immutable(DEFAULT_SIZE) in alive") public static void main(  String[] args){
