...
public class Webserver {
  ...
  @Perm(requires="unique(socketClient) in alive",ensures="unique(socketClient) in alive") public static void webserver() throws IOException {
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static void serveClient(  Socket socketClient) throws IOException {
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static void transfer(  OutputStream outStream,  File file) throws IOException {
    BufferedReader fileReader=new BufferedReader(new FileReader(file));
    Writer writer=new OutputStreamWriter(outStream);
    transferHeader(writer,file);
    transferData(writer,fileReader,file.length());
  }
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static void transferHeader(  Writer writer,  File file) throws IOException {
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static void transferData(  Writer outWriter,  Reader inReader,  long count) throws IOException {
    if (count > 0) {
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static void LOG(  String msg,  Object... args){
    System.out.println(String.format("LOG: " + msg,args));
  }
  public ServerSocket getMysocket(){
    Cloner cloner=new Cloner();
    mysocket=cloner.deepClone(mysocket);
    Cloner cloner=new Cloner();
    mysocket=cloner.deepClone(mysocket);
    return mysocket;
  }
  public static ReentrantReadWriteLock getSocketClientLock(){
    Cloner cloner=new Cloner();
    socketClientLock=cloner.deepClone(socketClientLock);
    Cloner cloner=new Cloner();
    socketClientLock=cloner.deepClone(socketClientLock);
    return socketClientLock;
  }
  public static Socket getSocketClient(){
    Cloner cloner=new Cloner();
    socketClient=cloner.deepClone(socketClient);
    Cloner cloner=new Cloner();
    socketClient=cloner.deepClone(socketClient);
    return socketClient;
  }
