  ...
  public static void createGraph(){
    try {
      ...
      for (      EClass _class : _listClasses) {
        ...
        dot.write("digraph stateTransitions { " + _class.getName() + "[shape=plaintext,style=bold, fontsize=14];");
        for (        EState state : _class.getStates())         dot.write(state.getName() + "[shape=box,fontsize=12];\n");
        for (        String transition : _class.getTransitions())         dot.write(transition + "\n");
        dot.write("}");
        dot.flush();
      }
    }
