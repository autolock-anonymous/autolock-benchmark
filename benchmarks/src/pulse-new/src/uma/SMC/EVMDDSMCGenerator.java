...
public class EVMDDSMCGenerator {
  ...
  public static void addBoolStateInvariant(  String variable,  String operator,  String value){
    if (PulseSettings.getInvariants() == 2 || PulseSettings.getInvariants() == 3 || PulseSettings.getFullModel() == 1) {
      if (operator.compareTo("==") == 0) {
        if (value.compareToIgnoreCase("NULL") == 0)         value="0";
 else         if (value.compareToIgnoreCase("FALSE") == 0)         value="0";
 else         if (value.compareToIgnoreCase("TRUE") == 0)         value="1";
      }
 else       if (operator.compareTo("!=") == 0) {
        if (value.compareToIgnoreCase("NULL") == 0)         value="1";
 else         if (value.compareToIgnoreCase("FALSE") == 0)         value="1";
 else         if (value.compareToIgnoreCase("TRUE") == 0)         value="0";
      }
      if (pkg.getClasses().getLast().getVariablesofBooleanInvariants().contains(variable) == false)       pkg.getClasses().getLast().getVariablesofBooleanInvariants().add(variable);
  ...
  public static String modifyConstructorSpecifications(  String prog){
    ...
    for (    EMethod _method : _class.getMethods()) {
      if (_method.getName().compareTo(_class.getName()) == 0) {
        ...
        if (req != -1 && ens != -1)         prog=prog.substring(0,req) + "ensures=\"Unique" + prog.substring(ens + 13,prog.length());
        return prog;
      }
    }
    return prog;
  ...
  public static void addEnsuresResultAPTS(  String ap,  String ts){
    ...
    EClass _class=pkg.getClasses().getLast();
    EMethod _method=_class.getMethods().getLast();
    EParameter _result=new EParameter("result",_method.getReturnType(),-1);
  ...
  public static void addStateInvariant(  String accessPermission,  String variable,  String state){
