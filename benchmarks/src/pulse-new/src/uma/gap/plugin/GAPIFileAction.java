...
public class GAPIFileAction implements IObjectActionDelegate {
  ...
  @SuppressWarnings("unused") private static final Logger log=Logger.getLogger(GAPIFileAction.class.getName());
  ...
  @Override public void run(  IAction action){
    ...
    int testType=-1;
    if (action.getId().endsWith(".alltest") == true)     testType=0;
 else     if (action.getId().endsWith("Sink.States"))     testType=1;
 else     if (action.getId().endsWith("Methods.Execution"))     testType=2;
 else     if (action.getId().endsWith("Adjaceny.Matrix"))     testType=3;
 else     if (action.getId().endsWith("Concurrency.Test"))     testType=4;
