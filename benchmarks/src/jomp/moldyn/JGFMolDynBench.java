...
public class JGFMolDynBench extends md implements JGFSection3 {
  ...
  @Perm(requires="unique(one) in alive",ensures="unique(one) in alive") public void JGFtidyup(){
  ...
  @Perm(requires="pure(ek) * pure(size) in alive",ensures="pure(ek) * pure(size) in alive") public void JGFvalidate(){
  ...
  public ReentrantReadWriteLock getEkLock(){
    Cloner cloner=new Cloner();
    ekLock=cloner.deepClone(ekLock);
    return ekLock;
  }
  ...
  public ReentrantReadWriteLock getOneLock(){
    Cloner cloner=new Cloner();
    oneLock=cloner.deepClone(oneLock);
    return oneLock;
  }
}
