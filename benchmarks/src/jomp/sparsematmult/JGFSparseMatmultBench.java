...
public class JGFSparseMatmultBench extends SparseMatmult implements JGFSection2 {
  ...
  int[] col;
  int[] row;
  @Perm(requires="pure(size) in alive",ensures="pure(size) in alive") public static void main(  String argv[]){
  ...
  @Perm(requires="full(size) in alive",ensures="full(size) in alive") public void JGFsetsize(  int size){
    R_col_lastElement_row_size_val_x_yLock.writeLock().lock();
    this.size=size;
    R_col_lastElement_row_size_val_x_yLock.writeLock().unlock();
  }
  @Perm(requires="share(lastElement) * pure(size) * share(x) * share(R) * unique(y) * unique(val) * unique(col) * unique(row) in alive",ensures="share(lastElement) * pure(size) * share(x) * share(R) * unique(y) * unique(val) * unique(col) * unique(row) in alive") public void JGFinitialise(){
    R_col_lastElement_row_size_val_x_yLock.writeLock().lock();
  ...
  @Perm(requires="pure(y) * pure(val) * pure(row) * pure(col) * pure(x) in alive",ensures="pure(y) * pure(val) * pure(row) * pure(col) * pure(x) in alive") public void JGFkernel(){
    R_col_lastElement_row_size_val_x_yLock.readLock().lock();
    SparseMatmult.test(y,val,row,col,x,SPARSE_NUM_ITER);
    R_col_lastElement_row_size_val_x_yLock.readLock().unlock();
  }
  @Perm(requires="pure(ytotal) * pure(size) in alive",ensures="pure(ytotal) * pure(size) in alive") public void JGFvalidate(){
  ...
  @Perm(requires="unique(x) in alive",ensures="unique(x) in alive") public void JGFtidyup(){
    R_col_lastElement_row_size_val_x_yLock.writeLock().lock();
    x=null;
    R_col_lastElement_row_size_val_x_yLock.writeLock().unlock();
    System.gc();
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") private static double[] RandomVector(  int N,  java.util.Random R){
  ...
  public static ReentrantReadWriteLock getR_col_lastElement_row_size_val_x_yLock(){
    Cloner cloner=new Cloner();
    R_col_lastElement_row_size_val_x_yLock=cloner.deepClone(R_col_lastElement_row_size_val_x_yLock);
    return R_col_lastElement_row_size_val_x_yLock;
  }
}
