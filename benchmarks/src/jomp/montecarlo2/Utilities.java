...
public final class Utilities {
  ...
  static String paths[];
  @Perm(requires="unique(paths) * immutable(DEBUG) in alive",ensures="unique(paths) * immutable(DEBUG) in alive") public static String which(  String executable,  String pathEnv){
  ...
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static String joinString(  String joinChar,  String stringArray[]){
    return joinStringOverloaded(joinChar,stringArray,0);
  }
  @Perm(requires="no permission in alive",ensures="no permission in alive") public static String joinStringOverloaded(  String joinChar,  String stringArray[],  int index){
  ...
  @Perm(requires="immutable(DEBUG) * none(className) in alive",ensures="immutable(DEBUG) * none(className) in alive") public static String[] splitString(  String splitChar,  String arg){
  ...
  public static ReentrantReadWriteLock getClassNameLock(){
    Cloner cloner=new Cloner();
    classNameLock=cloner.deepClone(classNameLock);
  ...
  public static ReentrantReadWriteLock getDEBUG_pathsLock(){
    Cloner cloner=new Cloner();
    DEBUG_pathsLock=cloner.deepClone(DEBUG_pathsLock);
    return DEBUG_pathsLock;
  }
  public static String getPaths(){
    Cloner cloner=new Cloner();
    paths=cloner.deepClone(paths);
    return paths;
  }
}
