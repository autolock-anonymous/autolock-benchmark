...
public class ToTask implements java.io.Serializable {
  ...
  @Perm(requires="unique(header) * unique(randomSeed) in alive",ensures="unique(header) * unique(randomSeed) in alive") public ToTask(  String header,  long randomSeed){
  ...
  @Perm(requires="pure(randomSeed) in alive",ensures="pure(randomSeed) in alive") public long getrandomSeed(){
    try {
      header_randomSeedLock.readLock().lock();
  ...
  @Perm(requires="full(randomSeed) in alive",ensures="full(randomSeed) in alive") public void setrandomSeed(  long randomSeed){
    header_randomSeedLock.writeLock().lock();
    this.randomSeed=randomSeed;
    header_randomSeedLock.writeLock().unlock();
  }
  public ReentrantReadWriteLock getHeader_randomSeedLock(){
    Cloner cloner=new Cloner();
    header_randomSeedLock=cloner.deepClone(header_randomSeedLock);
    return header_randomSeedLock;
  }
}
