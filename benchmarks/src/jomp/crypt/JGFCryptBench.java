...
public class JGFCryptBench extends IDEATest implements JGFSection2 {
  ...
  @Perm(requires="pure(array_rows) * pure(plain1) * pure(plain2) * pure(crypt1) in alive",ensures="pure(array_rows) * pure(plain1) * pure(plain2) * pure(crypt1) in alive") public void JGFvalidate(){
    boolean error;
    error=false;
  ...
  public ReentrantReadWriteLock getPlain2Lock(){
    Cloner cloner=new Cloner();
    plain2Lock=cloner.deepClone(plain2Lock);
    return plain2Lock;
  }
  public ReentrantReadWriteLock getCrypt1Lock(){
    Cloner cloner=new Cloner();
    crypt1Lock=cloner.deepClone(crypt1Lock);
    return crypt1Lock;
  }
  ...
  public ReentrantReadWriteLock getPlain1Lock(){
    Cloner cloner=new Cloner();
    plain1Lock=cloner.deepClone(plain1Lock);
    return plain1Lock;
  }
}
