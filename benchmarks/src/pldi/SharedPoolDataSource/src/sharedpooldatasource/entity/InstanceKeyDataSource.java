 ...
 */
  ...
  private int _timeBetweenEvictionRunsMillis=(int)Math.min((long)Integer.MAX_VALUE,GenericObjectPool.DEFAULT_TIME_BETWEEN_EVICTION_RUNS_MILLIS);
  private int _numTestsPerEvictionRun=GenericObjectPool.DEFAULT_NUM_TESTS_PER_EVICTION_RUN;
  private int _minEvictableIdleTimeMillis=(int)Math.min((long)Integer.MAX_VALUE,GenericObjectPool.DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS);
...
Cloner cloner=new Cloner();
jndiEnvironment=cloner.deepClone(jndiEnvironment);
String value=null;
...
Cloner cloner=new Cloner();
logWriter=cloner.deepClone(logWriter);
if (logWriter == null) {
...
/** 
 * When <tt>true</tt>, objects will be {*link PoolableObjectFactory#validateObject validated} before being returned by the {*link #borrowObject} method.  If the object fails to validate, it will be dropped from the pool, and we will attempt to borrow another.
 * @see #setTestOnBorrow
 */
...
/** 
 * When <tt>true</tt>, objects will be {*link PoolableObjectFactory#validateObject validated} before being returned to the pool within the {*link #returnObject}.
 * @see #setTestOnReturn
 */
...
/** 
 * Sets the number of objects to examine during each run of the idle object evictor thread (if any). <p> When a negative value is supplied, <tt>ceil({*link #numIdle})/abs({*link #getNumTestsPerEvictionRun})</tt> tests will be run.  I.e., when the value is <i>-n</i>, roughly one <i>n</i>th of the idle objects will be tested per run.
...
/** 
 * When <tt>true</tt>, objects will be {*link PoolableObjectFactory#validateObject validated} by the idle object evictor (if any).  If an object fails to validate, it will be dropped from the pool.
...
Cloner cloner=new Cloner();
validationQuery=cloner.deepClone(validationQuery);
return (this.validationQuery);
}
/** 
 * The SQL query that will be used to validate connections from this pool before returning them to the caller.  If specified, this query <strong>MUST</strong> be an SQL SELECT statement that returns at least one row.  Default behavior is to test the connection when it is borrowed.
 */
...
protected abstract PooledConnectionAndInfo getPooledConnectionAndInfo(String username,String password) throws SQLException ;
...
/** 
 * <CODE>Referenceable</CODE> implementation prepares object for binding in jndi.
 */
...
Cloner cloner=new Cloner();
cpds=cloner.deepClone(cpds);
return cpds;
}
public static String getBAD_TRANSACTION_ISOLATION(){
Cloner cloner=new Cloner();
BAD_TRANSACTION_ISOLATION=cloner.deepClone(BAD_TRANSACTION_ISOLATION);
return BAD_TRANSACTION_ISOLATION;
}
public String getInstanceKey(){
Cloner cloner=new Cloner();
instanceKey=cloner.deepClone(instanceKey);
return instanceKey;
}
public static String getGET_CONNECTION_CALLED(){
Cloner cloner=new Cloner();
GET_CONNECTION_CALLED=cloner.deepClone(GET_CONNECTION_CALLED);
return GET_CONNECTION_CALLED;
}
}
