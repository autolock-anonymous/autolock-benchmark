 ...
 */
  ...
  /** 
 * Creates a <code>BufferedInputStream</code> and saves its  argument, the input stream <code>in</code>, for later use. An internal buffer array is created and  stored in <code>buf</code>.
 ...
 */
  private void fill() throws IOException {
    if (markpos < 0)     pos=0;
 else     if (pos >= buf.length)     if (markpos > 0) {
      int sz=pos - markpos;
 ...
 */
  ...
  /** 
 * Returns the number of bytes that can be read from this input stream without blocking. <p> The <code>available</code> method of <code>BufferedInputStream</code> returns the sum of the the number of bytes remaining to be read in the buffer (<code>count&nbsp;- pos</code>) and the result of calling the <code>available</code> method of the underlying input stream.
 ...
 * @exception IOException  if this stream has not been marked orif the mark has been invalidated.
 */
 ...
 * @return  a <code>boolean</code> indicating if this stream type supportsthe <code>mark</code> and <code>reset</code> methods.
 */
