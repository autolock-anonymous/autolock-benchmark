 ...
 */
  ...
  /** 
 * Return the currently set  {@link ErrorHandler} for thisAppender.  
 * @since 0.9.0 
 */
  public ErrorHandler getErrorHandler(){
    Cloner cloner=new Cloner();
    errorHandler=cloner.deepClone(errorHandler);
 ...
 */
  public Layout getLayout(){
    Cloner cloner=new Cloner();
    layout=cloner.deepClone(layout);
    return layout;
  }
  /** 
 ...
 */
  public Priority getThreshold(){
    Cloner cloner=new Cloner();
    threshold=cloner.deepClone(threshold);
    return threshold;
  }
  /** 
 * Check whether the message level is below the appender's threshold. If there is no threshold set, then the return value is always <code>true</code>.
 */
...
/** 
 * Set the layout for this appender. Note that some appenders have their own (fixed) layouts or do not use one. For example, the {@link org.apache.log4j.net.SocketAppender} ignores the layout sethere. 
 */
...
/** 
 * Set the threshold level. All log events with lower level than the threshold level are ignored by the appender. <p>In configuration files this option is specified by setting the value of the <b>Threshold</b> option to a level string, such as "DEBUG", "INFO" and so on.
 * @since 0.8.3 
 */
...
Cloner cloner=new Cloner();
tailFilter=cloner.deepClone(tailFilter);
return tailFilter;
}
public Filter getHeadFilter(){
Cloner cloner=new Cloner();
headFilter=cloner.deepClone(headFilter);
return headFilter;
}
}
