package hashtablejava167.entity;
...
;
      ...
      public Object[] toArray(){
        Object[] a=c.toArray();
        for (int i=0; i < a.length; i++)         a[i]=new UnmodifiableEntry<K,V>((Entry<K,V>)a[i]);
        return a;
      }
      public <T>T[] toArray(      T[] a){
        Object[] arr=c.toArray(a.length == 0 ? a : Arrays.copyOf(a,0));
        for (int i=0; i < arr.length; i++)         arr[i]=new UnmodifiableEntry<K,V>((Entry<K,V>)arr[i]);
...
private static class UnmodifiableEntry<K,V> implements Entry<K,V> {
        ...
        public Entry<? extends K,? extends V> getE(){
          Cloner cloner=new Cloner();
...
synchronized (mutex) {
    ...
    public Set<Entry<K,V>> entrySet(){
synchronized (mutex) {
        if (entrySet == null)         entrySet=new SynchronizedSet<Entry<K,V>>(m.entrySet(),mutex);
        return entrySet;
      }
    }
    public Collection<V> values(){
...
;
      ...
      public Object[] toArray(){
        ...
        for (int i=0; i < source.length; i++)         dest[i]=new CheckedEntry<K,V>((Entry<K,V>)source[i],valueType);
        return dest;
      }
      public <T>T[] toArray(      T[] a){
        Object[] arr=s.toArray(a.length == 0 ? a : Arrays.copyOf(a,0));
        for (int i=0; i < arr.length; i++)         arr[i]=new CheckedEntry<K,V>((Entry<K,V>)arr[i],valueType);
        if (arr.length > a.length)         return (T[])arr;
        System.arraycopy(arr,0,a,0,arr.length);
        if (a.length > arr.length)         a[arr.length]=null;
        return a;
...
private static class SingletonMap<K,V> extends AbstractMap<K,V> implements Serializable {
    ...
    public Set<Entry<K,V>> entrySet(){
      if (entrySet == null)       entrySet=Collections.<Entry<K,V>>singleton(new SimpleImmutableEntry<K,V>(k,v));
      return entrySet;
    }
    public Collection<V> values(){
