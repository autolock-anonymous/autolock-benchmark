...
/** 
 * This class implements flow analyses over 3-address code instructions (  {@link TACInstruction}).  To define a specific analysis, implement   {@link ITACTransferFunction} for conventional or {@link ITACBranchSensitiveTransferFunction} for branch-sensitive flow analyses.
 * @author Kevin Bierhoff
 * @param < LE >	The LatticeElement subclass that represents the analysis knowledge
 */
public class TACFlowAnalysis<LE extends LatticeElement<LE>> extends MotherFlowAnalysis<LE> implements ITACFlowAnalysis<LE>, ITACAnalysisContext {
 ...
 */
  @Deprecated public TACFlowAnalysis(  Crystal crystal,  ITACTransferFunction<LE> transferFunction,  CompilationUnitTACs eclipseTAC){
    super(crystal);
    this.driver=new BranchInsensitiveTACAnalysisDriver<LE>(transferFunction,eclipseTAC);
    transferFunction.setAnalysisContext(this);
  }
 ...
 * @deprecated Use constructor that only takes transfer function.
 */
  @Deprecated public TACFlowAnalysis(  Crystal crystal,  ITACBranchSensitiveTransferFunction<LE> transferFunction,  CompilationUnitTACs eclipseTAC){
    super(crystal);
    this.driver=new BranchSensitiveTACAnalysisDriver<LE>(transferFunction,eclipseTAC);
    transferFunction.setAnalysisContext(this);
  }
  public LE getResultsAfter(  TACInstruction instr){
    ...
    if (rootInstr == instr) {
      ...
      LE incoming=this.driver.tf.getAnalysisDirection() == AnalysisDirection.BACKWARD_ANALYSIS ? getResultsOrNullAfter(node) : getResultsOrNullBefore(node);
      if (incoming == null)       return getResultsAfter(node);
 else       return mergeLabeledResult(this.driver.deriveResult(seq,incoming,instr,true),node);
    }
 else     throw new UnsupportedOperationException("Can't determine results for instruction: " + instr);
  }
  public LE getResultsBefore(  TACInstruction instr){
    ...
    if (rootInstr == instr) {
      ...
      EclipseInstructionSequence seq=(EclipseInstructionSequence)rootInstr;
      LE incoming=this.driver.tf.getAnalysisDirection() == AnalysisDirection.BACKWARD_ANALYSIS ? getResultsOrNullAfter(node) : getResultsOrNullBefore(node);
      if (incoming == null)       return getResultsBefore(node);
 else       return mergeLabeledResult(this.driver.deriveResult(seq,incoming,instr,false),node);
    }
 else     throw new UnsupportedOperationException("Can't determine results for instruction: " + instr);
  }
  public IResult<LE> getLabeledResultsAfter(  TACInstruction instr){
    ...
    if (rootInstr == instr) {
      ...
      EclipseInstructionSequence seq=(EclipseInstructionSequence)rootInstr;
      LE incoming=this.driver.tf.getAnalysisDirection() == AnalysisDirection.BACKWARD_ANALYSIS ? getResultsOrNullAfter(node) : getResultsOrNullBefore(node);
      if (incoming == null)       return getLabeledResultsAfter(node);
 else       return this.driver.deriveResult(seq,incoming,instr,true);
    }
 else     throw new UnsupportedOperationException("Can't determine results for instruction: " + instr);
  }
  public IResult<LE> getLabeledResultsBefore(  TACInstruction instr){
    ...
    if (rootInstr == instr) {
      ...
      EclipseInstructionSequence seq=(EclipseInstructionSequence)rootInstr;
      LE incoming=this.driver.tf.getAnalysisDirection() == AnalysisDirection.BACKWARD_ANALYSIS ? getResultsOrNullAfter(node) : getResultsOrNullBefore(node);
      if (incoming == null)       return getLabeledResultsBefore(node);
 else       return this.driver.deriveResult(seq,incoming,instr,false);
    }
 else     throw new UnsupportedOperationException("Can't determine results for instruction: " + instr);
  }
  ...
  /** 
 * Returns the TAC variable for a given ASTNode <i>after previously analyzing the method surrounding the given node.</i> It is the caller's responsibility to make sure to call this method only when analysis results for the surrounding method are available.
 ...
 */
  public Variable getVariable(  ASTNode node){
    if (hasResults(node) || findSurroundingMethod(node) == getCurrentMethod())     return driver.tac.variable(node);
    throw new IllegalArgumentException("Not currently analyzing method surrounding node: " + node);
  }
  /** 
 * Returns the <b>this</b> variable for a given method <i>after previously analyzing that method.</i> It is the caller's responsibility to make sure to call this method only when analysis results for the given method are available.
 ...
 */
  public ThisVariable getThisVariable(  MethodDeclaration methodDecl){
    if (methodDecl == null || methodDecl != getCurrentMethod())     throw new IllegalArgumentException("Not currently analyzing method: " + methodDecl);
  ...
  /** 
 * Returns the variable for a given parameter or local <i>after previously analyzing the method declaring the parameter or local.</i> It is the caller's responsibility to make sure to call this method only when analysis results for the declaring method  are available.
 ...
 */
  public SourceVariable getSourceVariable(  IVariableBinding varBinding){
    if (varBinding.getDeclaringMethod() == null)     throw new IllegalArgumentException("Not a local or parameter: " + varBinding);
    if (varBinding.getDeclaringMethod().equals(getCurrentMethod().resolveBinding()) == false)     throw new IllegalArgumentException("Not currently analyzing method declaring variable: " + varBinding);
...
protected abstract static class AbstractTACAnalysisDriver<LE extends LatticeElement<LE>,TF extends IFlowAnalysisDefinition<LE>> implements IFlowAnalysisDefinition<LE> {
    protected TF tf;
 ...
 */
    ...
    public CompilationUnitTACs getCompUnitTacs(){
      Cloner cloner=new Cloner();
      compUnitTacs=cloner.deepClone(compUnitTacs);
      Cloner cloner=new Cloner();
      compUnitTacs=cloner.deepClone(compUnitTacs);
      return compUnitTacs;
    }
    public TF getTf(){
      Cloner cloner=new Cloner();
      tf=cloner.deepClone(tf);
      Cloner cloner=new Cloner();
      tf=cloner.deepClone(tf);
      return tf;
    }
  }
protected static class BranchInsensitiveTACAnalysisDriver<LE extends LatticeElement<LE>> extends AbstractTACAnalysisDriver<LE,ITACTransferFunction<LE>> implements ITransferFunction<LE> {
    public BranchInsensitiveTACAnalysisDriver(    ITACTransferFunction<LE> tf,    CompilationUnitTACs compUnitTacs){
      super(tf,compUnitTacs);
    }
    public LE transfer(    ASTNode astNode,    LE incoming){
      LE result;
      TACInstruction instr=tac.instruction(astNode);
      if (instr == null)       result=incoming;
 else       result=instr.transfer(tf,incoming);
      return result;
    }
...
protected class BranchSensitiveTACAnalysisDriver<LE extends LatticeElement<LE>> extends AbstractTACAnalysisDriver<LE,ITACBranchSensitiveTransferFunction<LE>> implements IBranchSensitiveTransferFunction<LE> {
    public BranchSensitiveTACAnalysisDriver(    ITACBranchSensitiveTransferFunction<LE> tf,    CompilationUnitTACs compUnitTacs){
      super(tf,compUnitTacs);
    }
    public IResult<LE> transfer(    ASTNode astNode,    List<ILabel> labels,    LE value){
      TACInstruction instr=tac.instruction(astNode);
      if (instr == null)       return new LabeledSingleResult<LE>(value,labels);
 else       return instr.transfer(tf,labels,value);
    }
    public IResult<LE> deriveResult(    EclipseInstructionSequence seq,    LE incoming,    TACInstruction targetInstruction,    boolean afterResult){
      Set<ILabel> labels=tf.getAnalysisDirection() == AnalysisDirection.BACKWARD_ANALYSIS ? getLabeledResultsBefore(targetInstruction.getNode()).keySet() : getLabeledResultsAfter(targetInstruction.getNode()).keySet();
  ...
  public AbstractTACAnalysisDriver<LE,?> getDriver(){
    Cloner cloner=new Cloner();
    driver=cloner.deepClone(driver);
    Cloner cloner=new Cloner();
    driver=cloner.deepClone(driver);
    return driver;
  }
