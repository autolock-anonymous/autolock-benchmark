...
public class EclipseCFGNode implements ICFGNode {
  ...
  public Set<EclipseCFGEdge> getInputs(){
    ...
    inputs=cloner.deepClone(inputs);
    Cloner cloner=new Cloner();
    inputs=cloner.deepClone(inputs);
    return inputs;
  }
  public Set<EclipseCFGEdge> getOutputs(){
    Cloner cloner=new Cloner();
    outputs=cloner.deepClone(outputs);
    Cloner cloner=new Cloner();
    outputs=cloner.deepClone(outputs);
    return outputs;
  }
 ...
 * @return The output edges that have the ILabel label, or the empty setif non exist
 */
  public Set<EclipseCFGEdge> getInputEdges(  ILabel label){
    ...
    for (    EclipseCFGEdge edge : inputs) {
      if (edge.label instanceof BooleanLabel)       hasBooleanLabel=true;
 else       if (hasBooleanLabel)       throw new CrystalRuntimeException("Node has input labels that are booleans and others: " + node);
      if (edge.label.equals(label))       filteredEdges.add(edge);
    }
 ...
 */
  ...
  public String getName(){
    ...
    if (node == null)     return "";
    if (node.nodeClassForType(node.getNodeType()) == null)     throw new CrystalRuntimeException("Got a node without a type?!?");
    return node.nodeClassForType(node.getNodeType()).getSimpleName();
  }
  public EclipseCFGNode getStartNode(){
    Cloner cloner=new Cloner();
  ...
  public String getMyName(){
    Cloner cloner=new Cloner();
    myName=cloner.deepClone(myName);
    Cloner cloner=new Cloner();
    myName=cloner.deepClone(myName);
    return myName;
  }
  ...
  public EclipseCFGNode getEndNode(){
    Cloner cloner=new Cloner();
