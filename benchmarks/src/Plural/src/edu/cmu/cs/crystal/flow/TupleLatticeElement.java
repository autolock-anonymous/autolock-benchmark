...
/** 
 * Represents a lattice element that is a tuple of lattice elements for an underlying lattice. Conceptually, this is a map from keys to individual lattice elements for each key. Keys can be parameterized, but common keys are AST nodes and 3-address code   {@link edu.cmu.cs.crystal.tac.Variable}s.  The representation is optimized: a default element is the assumed value for every key that has not been explicitly set. <b>This lattice is mutable</b>; the   {@link #put(Object,LatticeElement)} operation can be used to change its value.
 * @author aldrich
 ...
 * @return bottom if this lattice is bottom,theDefault if n not found in map, or else the element at n in the map 
 */
  public LE get(  K n){
    if (elements == null)     return bot;
    LE elem=elements.get(n);
    if (elem == null)     return theDefault;
 else     return elem;
  }
  /** 
 ...
 * @return Lattice information previously stored in the tuple or <code>null</code>if element wasn't in the tuple.
 */
 ...
 */
  @Override public String toString(){
    ...
    for (    K key : keys) {
      ...
      isFirst=false;
      LE myLE=get(key);
      buf.append(key.getClass().getSimpleName()).append('{').append(key.toString()).append("}->").append(myLE.toString());
    }
    buf.append(']');
    return buf.toString();
  }
 ...
 * @return The set of keys for which lattice information was previously stored.The result is never <code>null</code> but may be empty.
 * @see java.util.Map#keySet()
 */
  public Set<K> getKeySet(){
    if (elements == null)     return new HashSet<K>();
 else     return elements.keySet();
  }
  /** 
 ...
 */
  ...
  protected HashMap<K,LE> joinMaps(  TupleLatticeElement<K,LE> le1,  TupleLatticeElement<K,LE> le2,  ASTNode node){
 ...
 */
  public boolean atLeastAsPrecise(  TupleLatticeElement<K,LE> other,  ASTNode node){
    ...
    for (    K key : keys) {
      ...
      if (!myLE.atLeastAsPrecise(otherLE,node))       return false;
    }
    return true;
  }
  /** 
 ...
 */
  public TupleLatticeElement<K,LE> copy(){
    if (elements == null)     return new TupleLatticeElement<K,LE>(bot,theDefault,null);
  ...
  public HashMap<K,LE> getElements(){
    Cloner cloner=new Cloner();
    elements=cloner.deepClone(elements);
    Cloner cloner=new Cloner();
    elements=cloner.deepClone(elements);
    return elements;
  }
  public LE getTheDefault(){
    Cloner cloner=new Cloner();
    theDefault=cloner.deepClone(theDefault);
    Cloner cloner=new Cloner();
    theDefault=cloner.deepClone(theDefault);
    return theDefault;
  }
