 ...
 */
  ...
  public ICFGNode getUberReturn(){
    Cloner cloner=new Cloner();
    uberReturn=cloner.deepClone(uberReturn);
    Cloner cloner=new Cloner();
    uberReturn=cloner.deepClone(uberReturn);
    return uberReturn;
  }
  ...
  /** 
 * Why might source be null? We get source from an endNode of something else. endNodes are allowed to be null if a node decides to handle their own outgoing control flow. This happens in breaks, returns, continues, throws, etc.
 */
 ...
 else       parent.setStart(current.getStart());
      last=current;
    }
    if (last != null) {
  ...
  /** 
 * Creates an edge DIRECTLY from source to the proper finally blocks, and then from the finally blocks DIRECTLY to the destination. By directly, this means that it won't call getEnd or getStart. It uses the exceptionToStopAt to determine which finally blocks to grab. It will also make the edge from the source to the finally block have this exception. A normal edge will be used when exceptionToStopAt is null
 */
  protected void hookFinally(  EclipseCFGNode source,  ITypeBinding exceptionToStopAt,  EclipseCFGNode dest){
    ...
    while (!finallyStack.isEmpty()) {
      ...
      if (first == null)       first=cloneCurrent;
      if (last != null)       createEdge(last.getEnd(),cloneCurrent.getStart());
      last=cloneCurrent;
    }
    if (first != null) {
      if (exceptionToStopAt == null)       createEdge(source,first.getStart());
 else       createEdge(source,first.getStart(),exceptionToStopAt);
      createEdge(last.getEnd(),dest);
    }
 else {
      if (exceptionToStopAt == null)       createEdge(source,dest);
 else       createEdge(source,dest,exceptionToStopAt);
    }
  }
 ...
 * @return
 */
  private EclipseCFGNode copySubgraphRecur(  EclipseCFGNode current,  EclipseCFGNode stopNode,  HashMap<EclipseCFGNode,EclipseCFGNode> cloneMap){
    ...
    if (current == stopNode || node instanceof ReturnStatement || node instanceof ThrowStatement || node instanceof BreakStatement || node instanceof ContinueStatement) {
      for (      EclipseCFGEdge edge : current.getOutputs()) {
        EclipseCFGEdge cloneEdge=new EclipseCFGEdge(clone,edge.getSink(),edge.getLabel());
    ...
    if (cloneMap.containsKey(start))     start=cloneMap.get(start);
    if (cloneMap.containsKey(end))     end=cloneMap.get(end);
  ...
  @Override public void endVisit(  ForStatement node){
 ...
 else       forEnd.setStart(current.getStart());
      last=current;
    }
    if (cond != null) {
      if (last != null)       createEdge(last.getEnd(),cond.getStart());
 else       forEnd.setStart(cond.getStart());
      createBooleanEdge(cond.getEnd(),forEnd,false);
      createBooleanEdge(cond.getEnd(),body.getStart(),true);
    }
 else {
      if (last != null)       createEdge(last.getEnd(),body.getStart());
 else       forEnd.setStart(body.getStart());
    }
    ...
    if (cond != null)     createEdge(last.getEnd(),cond.getStart());
 else     createEdge(last.getEnd(),body.getStart());
    blockStack.popUnlabeled();
  ...
  @Override public boolean visit(  MethodDeclaration node){
    ...
    if (node.isConstructor()) {
      ...
      for (      FieldDeclaration field : type.getFields()) {
        if (!Modifier.isStatic(field.getModifiers()))         field.accept(this);
      }
    }
    for (    ASTNode param : (List<ASTNode>)node.parameters())     param.accept(this);
    if (node.getBody() != null)     for (    ASTNode stmt : (List<ASTNode>)node.getBody().statements())     stmt.accept(this);
    return false;
  }
  @Override public void endVisit(  MethodDeclaration node){
    ...
    if (node.isConstructor())     body=setUpConstructorBody(node);
 else     body=setUpMethodBody(node);
 ...
 else     method.setStart(body.getStart());
    for (int ndx=0; ndx < node.thrownExceptions().size(); ndx++)     exceptionMap.popCatch();
 ...
 */
  private EclipseCFGNode setUpMethodBody(  MethodDeclaration node){
    EclipseCFGNode body=new EclipseCFGNode(node.getBody());
    if (node.getBody() != null)     makeListEdges(null,node.getBody().statements(),body);
    return body;
  }
  ...
  @Override public void endVisit(  SwitchStatement node){
    ...
    if (!hasDefault)     createEdge(exp.getEnd(),switchEnd,(Expression)null);
    blockStack.popUnlabeled();
  ...
  @Override public boolean visit(  TryStatement node){
    ...
    for (int ndx=0; ndx < node.catchClauses().size(); ndx++)     exceptionMap.popCatch();
    if (finallyNode != null) {
 ...
 else     createEdge(bodyNode.getEnd(),tryNode);
    for (    CatchClause catchClause : (List<CatchClause>)node.catchClauses()) {
      catchNode=nodeMap.get(catchClause);
      if (finallyNode != null)       createEdge(catchNode.getEnd(),finallyNode.getStart());
 else       createEdge(catchNode.getEnd(),tryNode);
    }
  ...
  private EclipseCFGNode handleVariableDecl(  VariableDeclaration node,  EclipseCFGNode startPoint){
    ...
    if (node.getInitializer() != null) {
      ...
      if (current != null)       createEdge(current,init.getStart());
 else       startPoint=init;
      current=init.getEnd();
    }
    if (current != null)     createEdge(current,name.getStart());
 else     startPoint=name;
  ...
  @Override public void endVisit(  ArrayCreation node){
    ...
    if (node.getInitializer() == null)     makeListEdges(null,node.dimensions(),arrayCreation);
 else {
  ...
  @Override public void endVisit(  InfixExpression node){
    ...
    if (op.equals(Operator.CONDITIONAL_AND) || op.equals(Operator.CONDITIONAL_OR)) {
      ...
      for (      Expression opNode : operands) {
        ...
        if (last != null)         createBooleanEdge(last.getEnd(),operand.getStart(),isAnd);
        createBooleanEdge(operand.getEnd(),infix,!isAnd);
        if (ndx == operands.size() - 1)         createBooleanEdge(operand.getEnd(),infix,isAnd);
        last=operand;
        ndx++;
      }
    }
  ...
  @Override public void endVisit(  ClassInstanceCreation node){
    ...
    for (    ITypeBinding exception : node.resolveConstructorBinding().getExceptionTypes()) {
      ...
      if (catchNode != null)       createEdge(constructor,catchNode.getStart(),exception);
    }
  ...
  @Override public void endVisit(  MethodInvocation node){
    ...
    makeListEdges(nodeMap.get(node.getExpression()),(List<ASTNode>)node.arguments(),invocation);
    for (    ITypeBinding exception : node.resolveMethodBinding().getExceptionTypes()) {
      EclipseCFGNode catchNode=exceptionMap.getCatchNode(exception);
      if (catchNode != null)       createEdge(invocation,catchNode.getStart(),exception);
    }
 ...
 else     field.setStart(name.getStart());
  }
 ...
 */
  ...
  public BlockStack<EclipseCFGNode> getBlockStack(){
    Cloner cloner=new Cloner();
  ...
  public Map<ITypeBinding,EclipseCFGNode> getExcpReturns(){
    Cloner cloner=new Cloner();
    excpReturns=cloner.deepClone(excpReturns);
    Cloner cloner=new Cloner();
    excpReturns=cloner.deepClone(excpReturns);
    return excpReturns;
  }
  ...
  public ExceptionMap<EclipseCFGNode> getExceptionMap(){
    Cloner cloner=new Cloner();
    exceptionMap=cloner.deepClone(exceptionMap);
    Cloner cloner=new Cloner();
    exceptionMap=cloner.deepClone(exceptionMap);
    return exceptionMap;
  }
  public HashMap<ASTNode,EclipseCFGNode> getNodeMap(){
    Cloner cloner=new Cloner();
  ...
  public EclipseCFGNode getUndeclExit(){
    Cloner cloner=new Cloner();
    undeclExit=cloner.deepClone(undeclExit);
    Cloner cloner=new Cloner();
    undeclExit=cloner.deepClone(undeclExit);
    return undeclExit;
  }
